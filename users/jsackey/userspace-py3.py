...
# userspace-p3.py - 	A user space tool to read/display loki records
#			Modified 08/31/2012 to work with Python 3
# 
...
import functools #reduce() no longer builtin for python 3

theData = [] #a global to hold all the data

#===================ARGPARSE===================
	#first make a parser obj
parser = argparse.ArgumentParser(description="A python3 tool to read loki generated Record files")
...
def get_map(arg):
    ...
    for line in lines:
        ...
        match = re.search("\t(.+[\w\d*])\s\s+(\w.*);.+/*\s+(\d+)\s+(\d+)",line.decode('utf-8'))
